...
find two distinct messages that hash to the same result. The adversary must find a pair of distinct inputs $X,X'$ satisfying
$h(X)=h(X')$; both inputs are chosen by the adversary.
...
\begin{figure}[htbp]\centering\input{fig/iterated-hash}\caption{Iterated Hash Functions}\label{fig:iterated-hash}\end{figure}
...
Research on iterated hash functions examines which properties of the
compression function are inherited by the complete hash. Collision
resistance and second-preimage resistance require separate reasoning.
The discussion below distinguishes length-dependent second-preimage
attacks from the collision-preservation theorem for a length-strengthened
construction. For the latter result the output transformation $g$ is the
identity function; a non-injective truncation must be analyzed separately.

\textbf{Compression-function security.} The relationship between a
compression function and the iterated hash depends on the attack model,
message length, padding, and allowed choice of chaining values. The historical
Lai--Massey discussion \cite{ref29} is not used here to claim a universal
$2^n$ second-preimage lower bound. In particular, long-message attacks
\cite{ref11} must also be considered.

\textbf{Theorem (Merkle--Damg\aa rd, length-strengthened form).}
Let $f:\{0,1\}^n\times\{0,1\}^b\to\{0,1\}^n$ be collision resistant.
Encode each supported message injectively into blocks $X_1,\ldots,X_t$,
including padding and its length, so that no valid encoding is a proper
suffix of another. With a fixed initial value, define
\begin{equation}\label{eq:2.2}
\begin{aligned}
H_0&=IV,\\
H_i&=f(H_{i-1},X_i),\qquad i=1,\ldots,t,\\
h(X)&=H_t.
\end{aligned}
\end{equation}
Here $t$ counts \emph{all} encoded blocks, including the length encoding.
The order of the two arguments of $f$ is state, then block in this formula;
\eqref{eq:2.1} uses block, then state. This consistent endpoint replaces the
original recurrence's undefined $H_{t+1}$.

To see why the construction preserves collision resistance, trace two equal
final states backwards. Unequal compression inputs giving an equal output
constitute a collision in $f$. Otherwise the last blocks and preceding states
must coincide, and this reasoning repeats. Equal-length encodings would then
be identical; unequal-length encodings would violate the suffix condition.
Thus distinct messages with equal hashes yield a compression-function
collision. Length fields must be unambiguous over the supported message
lengths; merely adding arbitrary padding is insufficient \cite{ref2,ref15,ref24}.
...
operations for long messages, assuming abundant memory. This is an attack-work estimate, not a lower bound on every possible attack.
