...
\(A=B+((A+\Phi(B,C,D)+X+K)\ll s),\)

where \emph{X} is the 512-bit input block (split into \emph{i} 32-bit
words), \emph{+} denotes addition modulo \(2^{32}\) , \(\Phi\) is the
round-dependent Boolean function (e.g.~XOR), \emph{K} is the
step-dependent constant, and \textless\textless{} \emph{s} denotes a
left circular shift by \emph{s} positions \cite{ref12}.

Rivest defined four 32-bit variables (referred to as chaining variables)
that are initialized to the following values:

\begin{equation}\label{eq:3.1}
\begin{aligned}A&=\mathtt{0x01234567}\\B&=\mathtt{0x89abcdef}\\C&=\mathtt{0xfedcba98}\\D&=\mathtt{0x76543210}\end{aligned}
\end{equation}
